  ...
  try {
    ...
    const result = await databases.listDocuments(
      ...
      [
        ...
        Query.lessThan("$createdAt", date7DaysAgo.toISOString()) // используем метаданные $createdAt
      ]
    );

    if (result.documents.length === 0) {
      const created = await createWeeklyReport(form);
      return created;
    }
...
export async function saveCompleted(userId, type, time, distance, exercises, description, coordinates) {
  const dataToSave = {
    userId: userId,
    typ: type,
    exercises: exercises,
...
export async function getRecommendedTrainings() {
  try {
    const trainings = await databases.listDocuments(
      ...
      [
        Query.orderDesc('$createdAt'), // сортируем по дате создания
        Query.limit(5) // ограничиваем до 20 документов
      ]
    );
